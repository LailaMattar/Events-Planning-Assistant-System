...
class HospitalityView extends StatefulWidget {
  final bool isHall;
  final VendorProfile vendorProfile;
  final bool isUser;
  const HospitalityView({Key? key,required this.isHall,required this.vendorProfile,required this.isUser}) : super(key: key);

  @override
  _HospitalityViewState createState() => _HospitalityViewState(isHall: isHall,vendorProfile: vendorProfile,isUser:isUser);
}

class _HospitalityViewState extends State<HospitalityView> {
  final bool isHall;
  final bool isUser;
  final VendorProfile vendorProfile;
  _HospitalityViewState({required this.isHall,required this.vendorProfile,required  this.isUser});
  List<String> images = [];
  int activePage = 0;
  @override
      ...
      children: [
        ...
        Visibility(
          ...
          child: Padding(
            ...
            child: BigButton(
              title: 'Post Hospitality',
              color: Colors.grey,
              onTap: () {
                Navigator.push(
                    context, SlideRightRoute(page: TypePostPage(isHall: isHall,type: 'hospitality',)));
              },
              Textcolor: Colors.white,
            ),
          ),
        ),
        vendorProfile.hospitality.isEmpty
            ? Center(
          child: Text(
            'No hospitality yet',
          ...
          child: ListView.builder(
            ...
            itemCount: vendorProfile.hospitality.length,
            shrinkWrap: true,
            itemBuilder: (context, index) {
              Post post = vendorProfile.hospitality[index];
              return Padding(
